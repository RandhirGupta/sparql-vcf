...
public class VCFConnection implements SailConnection {
    ...
    private final File vcfIndex;
	private final ValueFactory vf;

	public VCFConnection(File vcfFile, File vcfIndex, ValueFactory vf) {
		super();
	...
	public CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluate(
		...
		try {
			VCFTripleSource tripleSource = new VCFTripleSource(vcfFile, vcfIndex, vf);
	...
	public CloseableIteration<? extends Statement, SailException> getStatements(
		...
		final VCFFileFilterReader vcfFileFilterReader = new VCFFileFilterReader(
				vcfFile, vcfIndex, subj, pred, obj, contexts, vf);
		return new CloseableIteratorIteration<Statement, SailException>() {

			@Override
			public boolean hasNext() throws SailException {
				try {
					return vcfFileFilterReader.hasNext();
				} catch (QueryEvaluationException e) {
					throw new SailException(e);
				}
			}
			...
			public Statement next() throws SailException {
				try {
					return vcfFileFilterReader.next();
				} catch (QueryEvaluationException e) {
					throw new SailException(e);
				}
			}
	...
	public long size(Resource... contexts) throws SailException {
		for (Resource context : contexts)
			if (context == null) {
				final VCFFileFilterReader vcfFileFilterReader = new VCFFileFilterReader(
						vcfFile, vcfIndex, null, null, null, null, vf);
				long count = 0L;
				try {
					while (vcfFileFilterReader.hasNext()) {
						vcfFileFilterReader.next();
						count++;
					}
				} catch (QueryEvaluationException e) {
					throw new SailException(e);
	...
	// throw new SailException("VCF files can not be updated via SPARQL");
	// }
	//
	// @Override
		...
		return new CloseableIteratorIteration<Namespace, SailException>() {
			private Iterator<Namespace> namespaces = Arrays.asList(
					new Namespace[] {
							...
							new NamespaceImpl(VCF.PREFIX, VCF.NAMESPACE) })
					.iterator();
